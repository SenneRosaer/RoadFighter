...
ObserverScore::ObserverScore(std::shared_ptr<roadfighter::World> obj, std::shared_ptr<sf::RenderWindow> wind) : Observer(obj) {
    this->window = wind;
...
void ObserverScore::update(int score) {
        ...
        std::pair<double,double> t = Transformation::getInstance(window->getSize().x,window->getSize().y).Transform({2,0});
