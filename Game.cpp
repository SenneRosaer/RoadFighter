...
void Game::run()
{
        ...
        // Used to spawn the first car or obstacle before relying on a timer to make sure that the obstacles don't start
        // spawning to late
        bool first = true;
        bool first2 = true;
        ...
        // For caculating the framerate we use the clock at the start and then sleep at the end if the time used is less
        // than needed
                        ...
                        if (!Scoreboard) {
                                ...
                                if (first or Car1Timer <= 0) {
                                        if (world->getPlayer() != nullptr) {
                                                ...
                                                switch (random) {
                                                case 0:
                                                        // Only spawn when at a speed higher than the cars we should
                                                        // pass
                                                        if (world->getPlayer()->getSpeed() >
                                                                Config->getMaxSpeedPassingCar() &&
                                                            world->getPassingCars().size() < 7) {
                                                                first = false;
                                                                world->addPassingCar(factory->createPassingCar(
                                                                    Random::getInstance().getRandom()));
                                                ...
                                                case 1:
                                                        if (world->getPlayer()->getSpeed() >
                                                            ...
                                                            world->getMovingCars().size() < 4) {
                                                                first = false;
                                                                world->addMovingCar(factory->createMovingCar(
                                                                    Random::getInstance().getRandom()));
                                                                Car1Timer = Config->getCarSpawnTimer();
                                                        }
                                                        break;
                                                ...
                                                if (world->getPlayer() != nullptr) {
                                                        if (world->getPlayer()->getSpeed() >
                                                            Config->getMaxSpeedPassingCar()) {
                                                                ...
                                                                world->addRock(factory->createRock(
                                                                    Random::getInstance().getRandom3()));
                                                        }
                                                }
                                        } else {
                                                RockTimer--;
                                ...
                                if (bossCreated == true and world->getBoss() != nullptr) {
                                        ...
                                        bossLife.setOrigin(bossLife.getLocalBounds().width / 2,
                                                           bossLife.getLocalBounds().height / 2);
                                        bossLife.setString("Boss lifes: " +
                                                           std::to_string(world->getBoss()->getLife()));
                                        std::pair<double, double> t =
                                            Transformation::getInstance(window->getSize().x, window->getSize().y)
                                                .Transform({1.1, 0.5});
                                ...
                                textDistanceLeft.setString("Distance left: " +
                                                           std::to_string(world->getDistanceToNextLevel() / 100));
                                std::pair<double, double> t =
                                    Transformation::getInstance(window->getSize().x, window->getSize().y)
                                        .Transform({1.1, -0.5});
